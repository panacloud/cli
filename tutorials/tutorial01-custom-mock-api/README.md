...
#### Caution!
If you make a change to the graphql schema in `editable/src/graphql/schema/schema.graphql` and then execute the command `panacloud update`, your custom customMockLambdaLayer will not be updated. Instead, it will only change `types`, and the user will need to update his code accordingly.
