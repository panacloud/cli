# Move to Real APIs from Mock APIs
...
* Change the `is_mock` property of your respective lambda in editable src/panacloudconfig.json from `true` to `false` or run `panacloud config queryname --mock true` command(doing through command is recommended).In this example we will only change addUser query.
