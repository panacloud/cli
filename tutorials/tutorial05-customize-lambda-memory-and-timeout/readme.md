...
# Customize Lambda's memory size and timeout
...
Initially all lambda's memory size is set to 128 MB you can change it by customizing your lambda's memory size property in editable src/panacloudconfig.json from `128` to `1024` or run `panacloud config user --memory 1024` command(doing through command is recommended).After doing that just run `panacloud update command`.

## Change Timeout

Initially all lambda's timeout limit is set to 6 seconds you can change it by customizing your lambda's timeout property in editable src/panacloudconfig.json from `6` to `3` or run `panacloud config addUser --timeout 3` command(doing through command is recommended).After doing that just run `panacloud update command`.
